...
function parseCatalogSearchItems(response: unknown): CatalogSearchItem[] {
  try {
    const { items } = v.parse(catalogSearchResponseSchema, response)

    return items
  } catch (error) {
    globalThis.console.error('Catalog search response validation failed.', error)
...
function useCatalogSearch(isAuthenticated: Ref<boolean>) {
  ...
  const ready = useAsyncData('catalog-search', async (_app, { signal }): Promise<SearchOutcome> => {
    ...
    try {
      ...
      const items = parseCatalogSearchItems(response)
